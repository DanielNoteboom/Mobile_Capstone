...
def analyze_results(directory):
  ...
  for person in os.listdir(directory + "_test"):
    ...
    if os.path.isdir(directory + "_test/" + person):
      total += 1
      faces = facial_detection(directory + "_test" + "/" + person + "/1.jpg", 0, 0)
      if len(faces) != 1:
  ...
  print str(correct) + "/" + str(total) + " or " + str((correct * 1.0)/total) + "% accuracy correct"
  print str(top_three) + "/" + str(top_three) + " or " + str((correct * 100.0)/total) + "% accuracy for top three"
